...
| File | Role |
|------|------|
| `app.ts` | The app source (TypeScript). **Edit this.** |
| `app.js` | Compiled output. Generated by `tsc`; git-ignored (CI rebuilds it). |
| `index.html` | Markup + inline CSS; loads `app.js`. |
| `manifest.json` / `sw.js` / `icons/` | PWA shell. |
| `tsconfig.json` / `package.json` | TypeScript config and build scripts. |
